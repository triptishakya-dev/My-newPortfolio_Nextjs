  ...
  return (
    <section id="experience" className="px-8 md:px-16 py-20 pt-0">
      <p className="text-amber-400 text-xs font-semibold tracking-[0.2em] uppercase flex items-center gap-3 mb-4">
        Experience <span className="block w-10 h-px bg-amber-400" />
      </p>
      ...
      <div>
        {experiences.map((exp, i) => (
          <div key={i} className="grid grid-cols-1 md:grid-cols-[180px_1fr] gap-4 md:gap-12 py-10 border-t border-white/6 last:border-b group">
            ...
            <div>
              <div className="inline-flex items-center gap-2 text-xs font-semibold text-white bg-amber-500/10 border border-amber-500/20 px-3 py-1.5 rounded-sm mb-4">
              ...
              <p className="text-orange-400 text-sm mb-4 ml-5">{exp.company}</p>
              <p className="text-white/40 text-sm leading-relaxed mb-5 ml-5">{exp.desc}</p>
              <div className="flex flex-wrap gap-2 ml-5">
                {exp.pills.map((p) => (
                  <span key={p} className="text-xs px-3 py-1 bg-amber-500/8 border border-amber-500/20 text-amber-400 rounded-sm">{p}</span>
                ))}
              </div>
            </div>
          </div>
