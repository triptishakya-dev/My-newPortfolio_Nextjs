    ...
    <section id="projects" className="px-8 md:px-16 py-20">
      <div className="flex items-end justify-between mb-12">
        <div>
          <p className="text-amber-400 text-xs font-semibold tracking-[0.2em] uppercase flex items-center gap-3 mb-4">
            Projects <span className="block w-10 h-px bg-amber-400" />
          </p>
        ...
        <a href="https://github.com/triptishakya-dev" target="_blank" rel="noopener noreferrer"
          className="hidden md:inline-flex border border-white/8 hover:border-amber-500/30 text-white/40 hover:text-amber-400 font-display text-xs font-semibold tracking-widest uppercase px-5 py-3 rounded-sm transition-all duration-200">
          All on GitHub →
        </a>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-white/5">
        {projects.map((project) => (
          <div key={project.num}
            className={`group relative bg-[#0a0800] hover:bg-[#100c00] p-8 transition-colors duration-300 overflow-hidden ${project.featured ? "md:col-span-2" : ""}`}>
            {/* Accent line */}
            <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-amber-500 to-orange-400 scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left" />
            {project.featured && <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-amber-500 to-orange-400" />}
            ...
            <a href={project.link} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 text-xs font-bold tracking-widest uppercase text-amber-400 hover:gap-3 transition-all duration-200">
